...
namespace bwi_gridworld {
    ...
    int Grid::step(int agent_id, char direction) {
        if(validMove(agent_id, direction) ){
            ...
            //cout << agent_id << endl;
            outside++;
            if(rand() <= ((1 - agent_id*0.00001) * RAND_MAX)) {
                inside++;
                if(direction == 'n')
            ...
            //cout << "Rand: " << rand() << endl;
            //cout << "Rand ratio: " << (inside / outside) << endl;
    ...
    void printGrid(vector<Pos> agent_pos, vector<Pos> event_loc) {
        cout << "--------------------------------" << endl;
        for(int a = 9; a >= 0; a--) {
            for(int b = 0; b < 10; b++) {
                if(b == 0) {
                ...
                bool found = false;
                for(int c = 0; c < agent_pos.size(); c++) {
                    if(agent_pos.at(c).x == b && agent_pos.at(c).y == a) {
                        cout << " A ";
                        found = true;
                    }
                }
                
                for(int d = 0; d < event_loc.size(); d++) {
                    if(event_loc.at(d).x == b && event_loc.at(d).y == a && !found) {
                        cout << " E ";
                        found = true;
                    }
                }
    ...
    void Grid::runExperiments() {
        ...
        for(int i = 0; i < NUM_TESTS; i++) {
            for(step_count = 0; step_count < MAX_STEPS; ++step_count) {
                //printGrid(agent_positions, event_locations);
                next();
                //usleep(1000000);
            }
